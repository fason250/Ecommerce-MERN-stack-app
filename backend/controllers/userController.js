...
const signInUser = async(request,response)=>{
   try {
    ...
    if(!userExist) return response.json({success: false, msg:"user not found "})
    
    const isPasswordMatch = await verifyPassword(password, userExist.password)
    if(!isPasswordMatch) return response.json({success: false , msg: "Oops! invalid credentials try again!"})
...
const signUpUser = async(request,response) =>{
    try {
        ...
        const userExist = await userModel.findOne({email})
        if(userExist) return response.json({ success: false, msg: "user is Already exist"})

        //validating email and password
        if(!validator.isEmail(email)) return response.json({success: "false", msg: "please enter valid email"})
        if(password.length < 8) return response.json({success: false, msg: "please enter a strong password"})
        ...
        response.json({success: true, userToken: token})
